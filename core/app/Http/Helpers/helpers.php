...
function totalDeposit(){
    return Deposit::where('status',1)->sum('amount');
}

function totalWithdraw(){
    return Withdrawal::where('status', 1)->sum('amount');
}
...
function send_smtp_mail($config, $receiver_email, $receiver_name, $sender_email, $sender_name, $subject, $message)
{
    ...
    $f = fsockopen($credentials->host, $credentials->port);
...
function menuActiveForUser($routeName, $type = null)
{
    if ($type == 3) {
        $class = 'active';
    } elseif ($type == 2) {
...
function gatewayRedirectUrl(){
 ...
 function saveTreeLevelBonus($user, $charge, $bonus_percent, $level)
    {
        $general = GeneralSetting::first();
        $refUser = User::find($user->ref_by);
        if ($refUser) {
            $balance = $charge * $bonus_percent / 100;
            $refUser->balance += $balance;
            $refUser->save();

            $bonusHistory = new App\BonusHistory();
            $bonusHistory->user_id = $refUser->id;
            $bonusHistory->amount = $balance;
            $bonusHistory->currency_code = $general->cur_text;
            $bonusHistory->bonus_type = $level;
            $bonusHistory->save();
...
function giveLevelCommission($depositUser, $amount){

    if ($depositUser->ref_by) {
        $treeLevels = App\DepositLevelTree::where('status', 1)->orderBy('level_order', 'ASC')->get();
        if ($treeLevels) {
            $refByLevelUser = null;
            foreach ($treeLevels as $key => $level) {
                if ($key == 0) {
                    $refByLevelUser = $depositUser;
                }
                if ($refByLevelUser) {
                    $refByLevelUser = saveTreeLevelBonus($refByLevelUser, $amount, $level->percent, $level->title);
                }
            }
        }
    }
